...
export default function SaisieExerciceScreen({ route, navigation }) {
  ...
  const [commentaire, setCommentaire] = useState(
    route.params?.performancesExistantes?.commentaire || ''
  );
  ...
  const ajouterSerie = (indexUtilisateur) => {
    ...
    // 👉 on garde ton comportement d'origine : on copie le poids / reps de la dernière série
  ...
  // ❌ nouvelle fonction : supprimer une série
  const supprimerSerie = (indexUtilisateur, indexSerie) => {
    const copie = [...data];
    const userSeries = copie[indexUtilisateur];

    if (userSeries.length <= 1) {
      // si une seule série, on la reset plutôt que la supprimer complètement
      userSeries[0] = { poids: 0, repetitions: 8 };
    } else {
      userSeries.splice(indexSerie, 1);
    }

    copie[indexUtilisateur] = userSeries;
    setData(copie);
  };
        ...
        <View key={i} style={styles.utilisateurBloc}>
          ...
          {data[i].map((serie, j) => (
            <View key={j} style={styles.serieRow}>
              ...
              {/* ❌ bouton suppression série */}
              <TouchableOpacity onPress={() => supprimerSerie(i, j)}>
...
const styles = StyleSheet.create({
  ...
  lastPerfComment: { color: '#ccc', marginTop: 8, fontStyle: 'italic', fontSize: 13 },
  ...
  deleteIcon: { color: '#ff6666', fontSize: 18, marginLeft: 8 },
